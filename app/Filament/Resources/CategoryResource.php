...
class CategoryResource extends Resource
{
    ...
    protected static ?string $modelLabel = 'Thématique';
    protected static ?string $pluralModelLabel = 'Thématiques';
            ...
            Forms\Components\Section::make('Apparence')
                ->schema([
                    ...
                    Forms\Components\Toggle::make('is_pro')
                        ->label('Cible professionnelle (Entreprises)')
                        ->default(false),
                ]),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\DeleteAction::make()
                    ->before(function ($record, Tables\Actions\DeleteAction $action) {
                        if ($record->procedures()->exists() || $record->eservices()->exists() || $record->subcategories()->exists()) {
                            \Filament\Notifications\Notification::make()
                                ->warning()
                                ->title('Suppression impossible')
                                ->body('Cette thématique contient des services ou procédures. Veuillez les réaffecter avant de supprimer.')
                                ->persistent()
                                ->send();
                        ...
                        ->before(function (\Illuminate\Support\Collection $records, Tables\Actions\DeleteBulkAction $action) {
                            foreach ($records as $record) {
                                if ($record->procedures()->exists() || $record->eservices()->exists() || $record->subcategories()->exists()) {
                                    \Filament\Notifications\Notification::make()
                                        ->warning()
                                        ->title('Suppression groupée interrompue')
                                        ->body("La thématique '{$record->name}' contient des données liées.")
                                        ->persistent()
                                        ->send();
            ...
            ->emptyStateHeading('Aucune thématique')
            ->emptyStateDescription('Commencez par créer une thématique.')
            ->reorderable('order');
    }
